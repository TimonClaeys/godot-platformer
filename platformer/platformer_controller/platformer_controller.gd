...
func _ready():
	GameEvents.connect("player_hit_spikes", self, "_on_player_hit_spikes")
...
func calculate_release_gravity_multiplier(jump_velocity, min_jump_height):
	...
	var release_gravity : int = 0 - pow(jump_velocity, 2) / (2 * min_jump_height)
...
func _on_player_hit_spikes() -> void:
